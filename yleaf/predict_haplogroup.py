...
def get_most_likely_haplotype(
    ...
    for haplotype_name in sorted_depth_haplotypes:
        ...
        # but QC1 failed due to backbone noise or missing backbone markers.
        # Track the deepest such candidate with strong QC2+QC3.
        # When QC3 >= threshold (within-tree path confirmed), accept even 1 derived marker
        # since the tree path itself validates the prediction.
        if fallback_score is None and qc2_score >= treshold and (qc3_score >= treshold or qc3_score == 0):
            min_derived = 1 if qc3_score >= treshold and qc3_score > 0 else 2
            if haplotype_dict[node.name].nr_derived >= min_derived:
                ancestral_children = get_ancestral_children(node, haplotype_dict, tree)
                # When QC1=0 due to missing backbone data (not contradictory data),
                # use QC2*QC3 as effective score to avoid zero-product.
                effective_score = total_score if qc1_score > 0 else qc2_score * qc3_score
                fallback_score = (node.name, ancestral_children, qc1_score, qc2_score, qc3_score, effective_score, node.depth)
...
def get_ancestral_children(
    ...
    while len(to_cover) > 0:
        current_node = to_cover.pop()
        for name in current_node.children:
            if name in visited:
                continue
            visited.add(name)
...
def add_to_final_table(
    ...
    total_derived = sum(marker_linker.nr_derived for marker_linker in haplotype_dict.values())

    # Low_Y_Signal detection: only override when the prediction algorithm ALSO failed.
    # If the algorithm found a valid haplogroup (hg != "NA"), trust it — QC1/QC2/QC3
    # already validated the path. Samples with few but clean derived markers (e.g. 20 SNPs
    # cleanly tracing R-Z2103→R-M12149) should NOT be overridden.
    # Threshold of 5: truly female/empty samples have ~0-3 derived markers on known positions.
    if hg == "NA":
        if total_derived < 5:
            hg = f"Low_Y_Signal ({total_derived} SNPs)"
            final_table.append([folder.name, hg, "", total_reads,
                                valid_markers, "NA", "NA", "NA", "NA"])
        else:
            final_table.append([folder.name, hg, "", total_reads,
