...
### 🔄 Автообновление Y-Tree (YFull)

В проекте появился скрипт для автоматического скачивания последней версии дерева YFull (файл `current_tree.json`). 
Скрипт использует `ETag` (HTTP кэширование), поэтому он скачивает 3МБ файл дерева **только если на GitHub появилась новая версия**.

**Использование:**
```bash
# Ручной запуск (можно запускать перед процессом)
python3 update_yfull_tree.py
```

**Автоматизация через cron:**
Вы можете добавить его в cron для еженедельной проверки. Для этого выполните `crontab -e` и добавьте строку:
```bash
0 3 * * 0 cd /абсолютный/путь/к/Yleaf && python3 update_yfull_tree.py >> update.log 2>&1
```
